...
const channelOption = computed(() => ({
  tooltip:{trigger:'axis',backgroundColor:'rgba(0,0,0,0.9)',borderColor:'rgba(0,212,255,0.3)',textStyle:{color:'#e0e8f0',fontSize:14}},
  grid:{top:8,left:80,right:40,bottom:8},
  xAxis:{type:'value',splitLine:{lineStyle:{color:'rgba(0,212,255,0.04)',type:'dashed'}},axisLabel:{color:'#4a5a6a',fontSize:12}},
  yAxis:{type:'category',data:['支付宝','微信支付','特约商户','企业网银','手机银行'],axisLabel:{color:'#8899aa',fontSize:13},axisLine:{lineStyle:{color:'rgba(0,212,255,0.08)'}}},
  series:[
    {name:'交易金额',type:'bar',barWidth:10,data:[286,320,185,420,528],itemStyle:{color:{type:'linear',x:0,y:0,x2:1,y2:0,colorStops:[{offset:0,color:'#00d4ff'},{offset:1,color:'#00ffcc'}]}},label:{show:true,position:'right',color:'#00ffcc',fontSize:13,formatter:'{c}亿'}}
  ]
}))
...
const hourOption = computed(() => ({
  ...
  xAxis:{type:'category',data:['0-2','2-4','4-6','6-8','8-10','10-12','12-14','14-16','16-18','18-20','20-22','22-24'],axisLabel:{color:'#4a5a6a',fontSize:11,rotate:30},axisLine:{lineStyle:{color:'rgba(0,212,255,0.08)'}}},
...
.section-title { font-size:15px; font-weight:600; color:#00ffcc; padding:6px 12px; text-shadow:0 0 10px rgba(0,255,204,0.3); letter-spacing:2px; flex-shrink:0; border-bottom:1px solid rgba(0,212,255,0.06); &::before{content:'◆ ';color:#00d4ff} }
...
.top-rank { width:16px; text-align:center; font-size:11px; color:#8899aa; font-weight:600; }
.top-name { width:60px; font-size:12px; color:#e0e8f0; }
...
.top-count { width:40px; text-align:right; font-size:11px; color:#00ffcc; font-family:'Courier New',monospace; }
</style>